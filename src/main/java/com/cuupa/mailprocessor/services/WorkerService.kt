...
class WorkerService(private val runtimeService: RuntimeService,
    ...
    private fun setScanOrZipProperties(document: Document, properties: MutableMap<String, Serializable?>): Map<String, Serializable?> {
        when (document) {
            is Zip -> {
    ...
    private fun getScanProcessProperties(document: Document) = mutableMapOf(ProcessProperty.FILE_NAME.name to document.filename,
    ...
    private fun getEmailProperties(email: EMail) = mapOf(ProcessProperty.FILE_NAME.name to email.filename,
